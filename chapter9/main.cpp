...
int main(int argc, char *argv[]) {
    std::shared_ptr<DisplayImpl> impl1(new StringDisplayImpl("Hello, China"));
    std::shared_ptr<Display>     d1(new Display(impl1.get()));

    std::shared_ptr<DisplayImpl> impl2(new StringDisplayImpl("Hello, Tao"));
    std::shared_ptr<Display>     d2(new CountDisplay(impl2.get()));

    std::shared_ptr<DisplayImpl>  impl3(new StringDisplayImpl("Hello, Universe"));
